...
public class NaverMapSearchController {
        ...
        content =
            @Content(
                ...
                examples =
                    @ExampleObject(
                        value =
                            "{\"message\":\"네이버맵 응답 수집에 실패했습니다\",\"detail\":\"Timed out while waiting for Naver search response\"}")))
  })
  ...
  @Operation(
      summary = "좌표 기준 네이버맵 검색",
      description = "검색어와 중심 좌표(경도, 위도)를 기준으로 네이버맵 검색 결과 원본 데이터를 조회합니다.")
  @ApiResponses({
    @ApiResponse(
        responseCode = "200",
